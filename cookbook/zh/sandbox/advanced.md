...
| 典型启动时间 | < 100 ms            | < 200 ms               | < 1 s                                         | 数秒（取决于调度/拉取镜像）           | 数秒（取决于冷启动）   | 数秒（取决于冷启动）   | 数秒（取决于调度/拉取镜像）           | < 125 ms                    |
...
| 操作系统支持 | Linux/macOS/Windows | Linux/macOS            | macOS（Apple Silicon）、Linux（x86_64/ARM64） | Linux（集群节点）                     | 托管（云）             | 托管（云）             | 托管（云）                            | Linux（x86_64/ARM64）       |
