...
**AgentScope Runtime** 是一个面向 AI Agent 的全栈运行时，旨在解决两大核心挑战：**高效部署与服务化**以及**安全的沙箱化执行**。
...
=&\ \textbf{工具沙箱隔离}
+\ \textbf{AaaS API（Agent 即服务）}
+\ \textbf{可扩展部署} \\
&+\ \textbf{全栈可观测性（日志/链路追踪）}
+\ \textbf{框架兼容性}
\end{aligned}
$$
本 cookbook 将引导你使用 **AgentScope Runtime** 构建**可服务化**、面向生产环境的高质量 Agent 应用。
