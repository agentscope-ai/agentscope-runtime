...
class RedisMemoryService(MemoryService):
    ...
    def __init__(
        ...
        Args:
            ...
            ttl_seconds: Time-to-live in seconds for memory data. If None, data never expires (default: 3600, i.e., 1 hour)
            max_messages_per_session: Maximum number of messages per session. If None, no limit (default: None)
            health_check_interval: Interval in seconds for health checks on idle connections (default: 30.0).
                Connections idle longer than this will be checked before reuse. Set to 0 to disable.
            socket_keepalive: Enable TCP keepalive to prevent silent disconnections (default: True)
        """
    ...
    async def start(self) -> None:
        """Starts the Redis connection with proper timeout and connection pool settings."""
            ...
            for msg in msgs:
                ...
                if candidate_content:
                    ...
                    if any(keyword in msg_content_lower for keyword in keywords):
                        ...
                        # If we have top_k limit and already found enough, we can stop early
                        # (but we still need to check all sessions for completeness)
                        # Note: This is a simple optimization, for better results we might
                        # want to process all and then sort/rank
