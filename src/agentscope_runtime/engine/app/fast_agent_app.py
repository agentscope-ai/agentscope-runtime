...
class FastAgentApp(FastAPI, BaseApp, InterruptMixin):
    """
    High-performance Agent application integrating FastAPI and Runner
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.deployment_mode = DeploymentMode.DAEMON_THREAD
        self.protocol_adapters: List[Any] = self._init_protocol_adapters(
            app_name,
            app_description,
            a2a_config,
            agui_config,
        ...
        if interrupt_backend:
            self._init_interrupt_service(interrupt_backend)
        elif interrupt_redis_url:
            backend = RedisInterruptBackend(interrupt_redis_url)
    ...
    def _setup_builtin_routes(self):
        ...
        async def health_check():
            ...
            status = {"status": "healthy", "mode": self.deployment_mode.value}
    ...
    def _start_embedded_worker(self):
                ...
                for ep in self.custom_endpoints:
                    if ep.get("task_type"):
                        ...
                        if not hasattr(func, "celery_task"):
                            func.celery_task = self.register_celery_task(
                                func,
                                queue,
                            )
                # Get registered queues or use default
                queues = (
                    list(self.get_registered_queues())
                    if self.get_registered_queues()
                    else ["celery"]
                )
    ...
    def task(self, path: str, queue: str = "celery"):
        ...
        def decorator(func: Callable):
            ...
            return super().task(path, queue)(func)  # type: ignore[misc]
