...
from agentscope_runtime.adapters.crewai.memory import (
    create_crewai_session_history_memory,
)
    ...
    async def query_func(
        ...
        **kwargs,
        ...
        research_analyst = Agent(
            ...
            backstory=(
                "You are an expert analyst at a world-renowned research "
                "institute. You are known for your ability "
                "to break down complex questions and deliver "
                "well-structured, easy-to-understand answers."
            ),
            llm=llm,
        )
...
async def test_openai_compatible_mode(start_app):
    ...
    client = OpenAI(
        base_url=f"http://127.0.0.1:{PORT}/compatible-mode/v1",
    )
    ...
    data = resp.response
    assert "crew" in data["output"][-1]["content"][0]["text"].lower()
