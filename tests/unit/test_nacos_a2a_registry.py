...
class TestNacosRegistry:
    ...
    def test_register_without_sdk(self, agent_card, deploy_properties, transport_properties):
        """Test register() when SDK is not available."""
        with patch("agentscope_runtime.engine.deployers.adapter.a2a.nacos_a2a_registry._NACOS_SDK_AVAILABLE", False):
            registry = NacosRegistry()
            # Should not raise, just return early
            registry.register(agent_card, deploy_properties, transport_properties)
            assert registry._registration_status == RegistrationStatus.PENDING

    def test_register_without_port(self, mock_nacos_sdk, agent_card, transport_properties):
        """Test register() when port is not specified."""
        with patch("agentscope_runtime.engine.deployers.adapter.a2a.nacos_a2a_registry._NACOS_SDK_AVAILABLE", True):
            registry = NacosRegistry()
            deploy_props_no_port = DeployProperties(host="localhost", port=None)
            registry.register(agent_card, deploy_props_no_port, transport_properties)
            # Should not start registration task
            assert registry._register_task is None
            assert registry._register_thread is None

    def test_register_with_shutdown_requested(self, mock_nacos_sdk, agent_card, deploy_properties, transport_properties):
    ...
    @pytest.mark.asyncio
    async def test_register_with_running_loop(self, mock_nacos_sdk, agent_card, deploy_properties, transport_properties):
    ...
    def test_register_without_running_loop(self, mock_nacos_sdk, agent_card, deploy_properties, transport_properties):
        ...
        with patch("agentscope_runtime.engine.deployers.adapter.a2a.nacos_a2a_registry._NACOS_SDK_AVAILABLE", True):
            ...
            assert registry._register_thread.is_alive() or not registry._register_thread.is_alive()  # May have completed
    ...
    @pytest.mark.asyncio
    async def test_wait_for_registration_task(self, mock_nacos_sdk, agent_card, deploy_properties, transport_properties):
    ...
    @pytest.mark.asyncio
    async def test_wait_for_registration_timeout(self, mock_nacos_sdk, agent_card, deploy_properties, transport_properties):
        ...
        with patch("agentscope_runtime.engine.deployers.adapter.a2a.nacos_a2a_registry._NACOS_SDK_AVAILABLE", True):
            ...
            assert status in (RegistrationStatus.IN_PROGRESS, RegistrationStatus.PENDING)

    @pytest.mark.asyncio
    async def test_cleanup_with_task(self, mock_nacos_sdk, agent_card, deploy_properties, transport_properties):
        ...
        with patch("agentscope_runtime.engine.deployers.adapter.a2a.nacos_a2a_registry._NACOS_SDK_AVAILABLE", True):
            ...
            if registry._register_task:
                assert registry._register_task.cancelled() or registry._register_task.done()

    @pytest.mark.asyncio
    async def test_cleanup_with_wait(self, mock_nacos_sdk, agent_card, deploy_properties, transport_properties):
    ...
    @pytest.mark.asyncio
    async def test_register_to_nacos_with_shutdown(self, mock_nacos_sdk, agent_card):
        ...
        with patch("agentscope_runtime.engine.deployers.adapter.a2a.nacos_a2a_registry._NACOS_SDK_AVAILABLE", True):
            ...
            with patch(
                ...
                with patch(
                    ...
                    mock_nacos_sdk["builder"].server_address.assert_called_with("test.nacos.com:8848")
                    mock_nacos_sdk["builder"].username.assert_called_with("user")
                    mock_nacos_sdk["builder"].password.assert_called_with("pass")

    def test_register_duplicate_prevention(self, mock_nacos_sdk, agent_card, deploy_properties, transport_properties):
