...
async def test_ttl_refresh_on_write():
    ...
    try:
        ...
        assert ttl2 > 0 and ttl2 <= 2
        # TTL should be refreshed, so it should be longer than the remaining time after waiting
        # Since we waited 1.1 seconds, if TTL was not refreshed, remaining time should be < 1 second
...
async def test_ttl_disabled():
    ...
    try:
        ...
        # Verify key exists and has no TTL (TTL returns -1 when no expiration is set)
        ttl = await fake_redis.ttl(key)
